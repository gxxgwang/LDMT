...
def make_animation(source_image, driving_video, inpainting_network, kp_detector, dense_motion_network, avd_network,
    ...
    with torch.no_grad():
        ...
        driving0 = driving.cuda()
        kp_driving_initial = kp_detector(driving0)

        # driving_frame = driving0
        # driving_frame = driving_frame.to(device)
        kp_driving = kp_detector(driving0)
        if mode == 'standard':
            kp_norm = kp_driving
        elif mode == 'relative':
        ...
        kp_m = {}
        kp_m['fg_kp'] = 0.5 * kp_source['fg_kp'] + 0.5 * kp_driving['fg_kp']
        dense_motion = dense_motion_network(source_image=source, kp_driving=kp_m,
                                            kp_source=kp_source, bg_param=None,
                                            dropout_flag=False)
        out = inpainting_network(source, dense_motion)

        dense_motion2 = dense_motion_network(source_image=out['prediction'], kp_driving=kp_driving,
                                            kp_source=kp_m, bg_param=None,
                                            dropout_flag=False)
        out2 = inpainting_network(out['prediction'], dense_motion2)

        predictions_m.append(np.transpose(out['prediction'].data.cpu().numpy(), [0, 2, 3, 1])[0])
    ...
    return predictions_m, predictions
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", default='taichi.pth.tar', help="path to checkpoint to restore")

    parser.add_argument("--source_image", default='C:\\Users\\louyu\\Desktop\\TPS2\\exp\\source.png',
                        help="path to source image")
    parser.add_argument("--driving_video", default='C:\\Users\\louyu\\Desktop\\TPS2\\exp\\w81Tr0Dp1K8#002125#002449.mp4', help="path to driving video")
    parser.add_argument("--result_video", default='C:\\Users\\louyu\\Desktop\\TPS2\\exp\\res2', help="path to output")
    ...
    i = 0
    j = 0
    for dir in os.listdir(opt.driving_video):
        img_dir = os.path.join(opt.driving_video, dir)
        driving_video = imageio.imread(img_dir)
        if opt.cpu:
        ...
        if opt.find_best_frame:
            ...
            predictions_m, predictions = make_animation(source_image, driving_video, inpainting, kp_detector,
                                              dense_motion_network,
                                              avd_network, device=device, mode=opt.mode)
        # for frame in predictions_m:
        #     imageio.imsave(os.path.join(opt.result_video, "m"+"{:05d}.png".format(i)), img_as_ubyte(frame))
        #     i = i + 1

        for frame in predictions:
            imageio.imsave(os.path.join(opt.result_video, "f"+"{:05d}.png".format(j)), img_as_ubyte(frame))
            j = j + 1
