...
class Logger:
    ...
    def visualize_rec(self, inp, out, out2):
        image = self.visualizer.visualize(inp['driving'], inp['source'], out, out2)
    ...
    def log_epoch(self, epoch, models, inp, out, out2):
        self.epoch = epoch
        self.models = models
        if (self.epoch + 1) % self.checkpoint_freq == 0:
            self.save_cpk()
        self.log_scores(self.names)
        self.visualize_rec(inp, out, out2)
...
class Visualizer:
    ...
    def visualize(self, driving, source, out, out2):
        images = []
        ...
        # kp_middle1 = out['kp_middle1']['fg_kp'].data.cpu().numpy()
        mid_img1 = out['prediction'].data.cpu().numpy()
        mid_img1 = np.transpose(mid_img1, [0, 2, 3, 1])
        # images.append((mid_img, kp_middle1))
        images.append(mid_img1)
        
        # mid_img1 = out['mid_img_stod'].data.cpu().numpy()
        # mid_img1 = np.transpose(mid_img1, [0, 2, 3, 1])
        # # images.append((mid_img, kp_middle1))
        # images.append(mid_img1)
        
        # mid_img2 = out2['prediction'].data.cpu().numpy()
        # mid_img2 = np.transpose(mid_img2, [0, 2, 3, 1])
        # images.append(mid_img2)
        
        # Driving image with keypoints
        kp_driving = out['kp_driving']['fg_kp'].data.cpu().numpy()#base-out2,other-out
        ...
        # if 'deformed' in out2:
        #     deformed = out2['deformed'].data.cpu().numpy()
        #     deformed = np.transpose(deformed, [0, 2, 3, 1])
        #     images.append(deformed)

        # Result with and without keypoints
        prediction = out2['prediction'].data.cpu().numpy()
        prediction = np.transpose(prediction, [0, 2, 3, 1])
        images.append(prediction)
        
        
        if 'prediction3' in out2:
            prediction = out2['prediction3'].data.cpu().numpy()
            prediction = np.transpose(prediction, [0, 2, 3, 1])
            images.append(prediction)
        
        # fianl_img = out['final_image_stod'].data.cpu().numpy()
        # fianl_img = np.transpose(fianl_img, [0, 2, 3, 1])
        # images.append(fianl_img)
        
        # prediction = out2['prediction2'].data.cpu().numpy()
        # prediction = np.transpose(prediction, [0, 2, 3, 1])
        # images.append(prediction)
        ...
        # if 'occlusion_map' in out2:
        #     for i in range(len(out2['occlusion_map'])):
        #         occlusion_map = out2['occlusion_map'][i].data.cpu().repeat(1, 3, 1, 1)
        #         occlusion_map = F.interpolate(occlusion_map, size=source.shape[1:3]).numpy()
        #         occlusion_map = np.transpose(occlusion_map, [0, 2, 3, 1])
        #         images.append(occlusion_map)
